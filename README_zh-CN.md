...
- 如果存在标签笔记（通过前置标记的别名进行匹配），则直接打开该标签笔记
- 如果只有一个笔记包含该标签，则直接打开该笔记
- 如果多个笔记包含该标签，则显示一个选择列表供用户选择
...
aliases: ["#标签名称", "abc", "哈哈"]---
```
在搜索此标签时，该插件会优先打开此标签下的标签笔记。
# 开发信息
此插件基于[R. W. Blickhan](https://github.com/rwblickhan) 的[原始作品](https://github.com/rwblickhan/obsidian-tag-search)，并由Tiancheng Tan进行了修改和优化。
### 技术栈
